...
1. **Build nixos:** x86_64-linux
1. **Build alta:** aarch64-linux (via QEMU)
1. **Build macos:** aarch64-darwin
...
1. **ALWAYS run `nix flake check` before pushing**
1. **Use `nix fmt -- --check .` to verify formatting**
1. **If CI fails on formatting, fix with `nix fmt` and push**
1. **If flake check fails, debug the specific build target**
