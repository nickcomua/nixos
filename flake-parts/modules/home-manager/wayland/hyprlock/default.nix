{
  ...
  catppuccinCfg = config.catppuccin.hyprland;
in {
  ...
  programs.hyprlock = {
    enable = true;
    settings = {
      inherit (cfg.hyprlock) auth;
      source = [
        "${sources.hyprland}/themes/${catppuccinCfg.flavor}.conf"
        (builtins.toFile "hyprland-${catppuccinCfg.accent}-accent.conf" ''
          $accent=''$${catppuccinCfg.accent}
          $accentAlpha=''$${catppuccinCfg.accent}Alpha
        '')
      ];
      "$font" = "Recursive";
      general = {
